...
_T_트레일발동 = float(os.environ.get('TB_TRAILARM', '3.0'))    # 트레일 발동 최소고점 % - 고점이 이 값 미달이면 트레일 미적용(손절만).
#                                                              고점이 트레일% 이하인 거래는 트레일 청산 시 청산가가 본전 이하로 확정(고점+2.9% -> 스탑=진입×0.998).
#                                                              매수세 진입은 급등 순간이라 고점이 진입 직후 형성 -> 팝 반납이 손실일의 구조적 원인 (0=발동조건 없음)
_T_본전보호 = float(os.environ.get('TB_BREAKEVEN', '1'))       # 1이면 트레일 발동 후 스탑 하한을 본전(+비용)으로 고정 (수익권 진입 후 손실 전환 차단)
_T_비용 = float(os.environ.get('TB_COST', '0.35'))           # 왕복 거래비용 % (본전보호 하한 계산용, 백테스팅과 동일)
...
class TraderBot:
    ...
    def _judge_바(self, s_종목코드, dic_지표, n_완결초, n_주문참고가):
        ...
        if s_종목코드 in _T_디버그종목:
            ...
            b_c3, b_c4 = n_전체60 >= _T_최소거래량, n_이격률 >= _T_이격최소
        ...
        if dic_포지션['상태'] == '보유':
            ...
            # 트레일은 고점이 발동임계를 넘긴 뒤부터 적용 (미달 구간은 고정손절만) + 발동 후엔 본전(+비용) 하한
            b_무장 = n_피크가 >= dic_포지션['매수가'] * (1 + _T_트레일발동 / 100)
            n_트레일가 = n_피크가 * (1 - _T_트레일 / 100)
            if _T_본전보호:
                n_트레일가 = max(n_트레일가, dic_포지션['매수가'] * (1 + _T_비용 / 100))
            b_트레일 = b_무장 and n_판정가 <= n_트레일가
