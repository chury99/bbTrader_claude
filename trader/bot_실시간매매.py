...
_T_리스크비율 = float(os.environ.get('TB_RISK_PCT', '2.0'))    # 덩어리 대비 감내 리스크 예산 % (손절 시 최대 손실 한도, TB_STOP과 별개)
_T_주문금액 = int(os.environ.get('TB_ORDERAMT', '500000'))     # 예수금 조회 실패 시 폴백 주문금액 (원)
...
class TraderBot:
    ...
    def __init__(self, queue_mp_수신2매매=None):
        ...
        # 로그 기록
        self.make_로그(f'구동 시작 - 리스크사이징(예수금×{_T_투입비율:.0f}% 덩어리, 리스크 {_T_리스크비율:.1f}%/손절 {_T_손절:.1f}%), '
                     f'최대보유 {_T_최대보유종목}종목')
    ...
    # -----------------------------------------------------------------
    def _calc_매수금액(self):
        """ 리스크 기반 주문금액 산정 (진입 신호 시점의 예수금 조회)
            덩어리     = 예수금 × 투입비율%(50%)  -> 남은 예수금을 2로 나눈 한 덩어리
            리스크예산 = 덩어리 × 리스크비율%(2%) -> 이 포지션에서 감내할 최대 손실
            매수금액   = 리스크예산 ÷ 손절률%      -> 손절(TB_STOP) 시 손실이 리스크예산 이내가 되도록
                       = 덩어리 × (리스크비율 / 손절률)
            (손절 2% + 리스크 2% 이면 매수금액 = 덩어리 = 예수금의 50%. 손절이 리스크보다 크면 축소됨)
            예수금 조회 실패 시 폴백 주문금액(_T_주문금액) 사용 """
        try:
            dic_계좌잔고, _ = self.restapi.tr_체결잔고요청()
            n_예수금 = int(dic_계좌잔고.get('n_d2예수금', 0))
        except Exception as e:
            self.make_로그(f'예수금 조회 실패 - 폴백 주문금액 {_T_주문금액:,}원 사용 - {e}')
            return _T_주문금액
        if n_예수금 <= 0:
            return 0
        n_덩어리 = n_예수금 * _T_투입비율 / 100
        n_배수 = (_T_리스크비율 / _T_손절) if _T_손절 > 0 else 1.0   # 손절 0 방어
        n_매수금액 = int(n_덩어리 * n_배수)
        return min(n_매수금액, n_예수금)   # 예수금 초과 방지
