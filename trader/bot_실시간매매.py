...
_T_최대보유 = int(os.environ.get('TB_MAXHOLD', '1200'))       # 최대 보유시간 (초) - 청산신호 없이 초과 시 강제 타임아웃 매도
# 종목선정 (전일 일봉 기준)
...
class TraderBot:
    ...
    def _judge_바(self, s_종목코드, dic_지표, n_완결초, n_주문참고가):
        ...
        if dic_포지션['상태'] == '보유':
            ...
            b_보유초과 = n_경과 >= _T_최대보유      # 청산신호 없이 최대보유 초과 - 강제 타임아웃
            b_마감 = n_완결초 >= self.n_장마감초
            if b_손절 or b_소멸 or b_보유초과 or b_마감:
                # 매도 실패 시 상태가 '보유'로 남아 매 완결초 재호출되는 폭주 방지 (재시도 최소간격)
                if n_완결초 - dic_포지션.get('매도시도초', 0) >= _T_매도재시도간격:
                    dic_포지션['매도시도초'] = n_완결초
                    s_사유 = '손절' if b_손절 else '소멸' if b_소멸 else '보유초과' if b_보유초과 else '마감'
    ...
    def _send_매수(self, s_종목코드, dic_포지션, n_초, n_현재가):
        ...
        if isinstance(res, dict) and res.get('return_code') == 0:
            dic_포지션.update(상태='매수중', 주문초=n_초, 매수초=n_초, 매수가=n_현재가, 매도재시도=0,
                            주문수량=n_수량, 매도시도초=0)   # 주문수량=누적체결 상한, 매도시도초 리셋
            dic_포지션['진입횟수'] += 1
