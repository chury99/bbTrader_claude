...
class TraderBot:
    ...
    def __init__(self):
        ...
        # 보유종목 확인
        # self.dic_계좌잔고, self.df_종목별잔고 = self.api.tr_체결잔고요청()
        # self.dic_종목코드2종목명 = self.df_종목별잔고.set_index('종목코드')['종목명'].to_dict()
    ...
    def avtivate_종목감시(self):
        ...
        while True:
            ...
            # if True:
                ...
                # li_매도종목 = [종목코드 for 종목코드 in df_매도신호.loc[df_매도신호['매도신호'] == True, '종목코드']]
                li_매도종목 = df_매도신호.loc[df_매도신호['매도신호'] == True, '종목코드'].to_list()
                b_매도신호 = len(li_매도종목) > 0

                # 로그 기록
                li_매도종목명 = [self.dic_종목코드2종목명.get(종목코드) for 종목코드 in li_매도종목]
                self.make_로그(f'매도신호 탐색 - 매도대상 {li_매도종목명}')
            ...
            if b_매도신호:
                ...
                for s_종목코드 in li_매도종목:
                    ...
                    # 로그 기록
                    self.make_로그(f'매도주문 완료 - {s_종목명}({s_종목코드}) {n_주문단가:,.0f}원 {n_주문수량:,.0f}주')
    ...
    def check_매도신호(self, li_대상종목):
        """ 보유 종목의 매도신호 확인해서 리턴 """
        # 종목별 데이터 확인
        # li_대상종목 = self.df_종목별잔고['종목코드'].tolist()
        li_dic매도신호 = list()
        for s_종목코드 in li_대상종목:
            ...
            n_저가3봉 = df_분봉['저가'].values[-4:-1].min()
            n_당일고가 = df_분봉['고가'].max()
            ...
            # 매도신호 확인 - 익절
            n_익절기준가 = n_당일고가 - 2 * n_ATR14
            b_고가이탈 = n_종가1 < n_익절기준가 if n_ATR14 is not None else n_종가1 < n_저가3봉
            b_익절 = (n_고가수익률 > 10) and b_고가이탈

            # 매도신호 확인 - 손절
            n_손절기준가 = n_매수가 * (100 - 3 - 0.2) / 100
            b_손절 = n_종가1 < n_손절기준가

            # 결과 정리
            b_매도신호 = b_익절 or b_손절
            dic_매도신호 = dict(종목코드=s_종목코드, 종목명=s_종목명, 매도신호=b_매도신호, 익절신호=b_익절, 손절신호=b_손절)
            dic_매도신호.update(매수가=n_매수가, 종가1=n_종가1, 수익률=n_수익률, 익절기준가=n_익절기준가, 손절기준가=n_손절기준가,
                            저가3봉=n_저가3봉, 당일고가=n_당일고가, 고가수익률=n_고가수익률, ATR14=n_ATR14)
            li_dic매도신호.append(dic_매도신호)

        # df 정리
        df_매도신호 = pd.DataFrame(li_dic매도신호)
