...
# ─── 구간1 (개장 09:00~09:10) : 흐름추종 진입 + 트레일 청산 ───────────────────────────
...
#                                                              재진입 전수검증: 구간1 2회차 7건이 2승5패·합계 -5.38%, 계좌 +14.78%→+16.38% (7일 중 5일 개선)
#                                                              구간1은 창 600초·쿨다운180·평균보유347초라 어차피 최대 2건 → 상한1은 "종목당 하루 한 번"
#                                                              부수효과: 재진입이 없으므로 _구간1_쿨다운이 사문화됨 (0~300초 결과 동일)
...
_T_매도슬립 = float(os.environ.get('TB_SELLSLIP', '0.08'))    # 청산 체결 슬리피지 % (실측 반영) - 0.15→0.08 (2026-08-04 재실측)
#                                                              최초값 0.15는 2026-07-28 실전 6건 기준. 그 뒤 매도를 IOC지정가(현재가-5호가)로
#                                                              전환했으므로 재실측 - 주문체결 원장·실시간매매 로그·틱 3중 교차, 매도 42건/9거래일:
#                                                                완결초 → 주문시점 가격이동  +0.027%p
#                                                                주문시점 → 실제 체결       +0.009%p  (IOC지정가 전환 효과 - 사실상 0)
#                                                                합계 = 실측 +0.036% (부트스트랩 95% [-0.075, +0.137]) → 가정 0.15%는 구간 밖(과대)
#                                                              점추정치 0.036 을 그대로 쓰지 않고 0.08 로 둔 이유: 표본 42건이라 값이 아직 흔들리고,
#                                                              이 방향(낮추기)은 백테를 낙관 쪽으로 미는 방향이라 보수적으로 잡는다. 표본 축적 시 재조정.
#                                                              ※ 매수슬립은 실측 +0.183%(95% [+0.076, +0.314])로 가정 0.20% 가 구간 안 → 유지.
#                                                                매수 비용의 대부분은 호가가 아니라 판정→주문 지연 중 가격이동(+0.161%p)이다.
...
class AnalyzerBot:
    ...
    def _make_거래_종목(self, df_종목, s_일자, s_종목코드, s_종목명, n_전일고가):
        ...
        # 구간1/구간2는 시간대가 분리되며, 종목당 포지션은 하나만 유지 (실시간매매와 동일한 단일 포지션 북)
        li_dic거래 = list()
        n_길이 = len(ary_초)
        ...
        while True:
            ...
            # 스탑 레벨은 체결 시 매도슬립을 다시 먹으므로 그만큼 올려 잡아야 실현 손익이 0 (미반영 시 -_T_매도슬립)
            if _T_본전발동 > 0 and not b_구간1:
                n_본전가 = n_매수가 * (1 + _T_비용 / 100) / (1 - _T_매도슬립 / 100)
                ary_스탑 = np.where(ary_피크 >= n_매수가 * (1 + _T_본전발동 / 100),
            ...
            i_보유초과 = n_최대보유 - 1     # 진입 후 최대보유초 경과 시점 (실시간매매 경과>=최대보유와 등가)
