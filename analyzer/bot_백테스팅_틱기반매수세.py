...
import ut
from analyzer import bot_백테스팅_돌파매매 as _돌파매매
...
class AnalyzerBot(_돌파매매.AnalyzerBot):
    """ 틱 데이터 기반 매수세 전략 백테스팅 (벡터연산)
        파이프라인: pick_종목선정 → make_매매정보 → make_거래내역 → make_결과정리(상속) → make_매매일보
        - 진입: 매수세 형성 (60초 순매수비율/거래강도) + 눌림(고가 대비 이격) 필터
        - 청산: 매수세 소멸 (최소보유 이후) / 손절 / 장마감 """

    def __init__(self, n_검증일수=60, b_디버그모드=False):
        # 부모 초기화 (config/로그/카카오/도구)
        super().__init__(n_검증일수=n_검증일수, b_디버그모드=b_디버그모드)
        ...
        # 기준정보 정의 - 틱 데이터는 20260716부터 유효 (일 전체 수집 시작일)
