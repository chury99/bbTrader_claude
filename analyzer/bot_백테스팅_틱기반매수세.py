...
class AnalyzerBot:
    ...
    def _make_거래_종목(self, df_종목, s_일자, s_종목코드, s_종목명, n_전일고가):
        ...
        while len(li_dic거래) < _T_일최대거래:
            ...
            ary_무장 = ary_피크 >= n_매수가 * (1 + _T_트레일발동 / 100)
            ary_트레일가 = ary_피크 * (1 - _T_트레일 / 100)
            if _T_본전보호:
                ary_트레일가 = np.maximum(ary_트레일가, n_매수가 * (1 + _T_비용 / 100))
            ary_스탑 = np.where(ary_무장, np.maximum(n_손절가, ary_트레일가), n_손절가)
