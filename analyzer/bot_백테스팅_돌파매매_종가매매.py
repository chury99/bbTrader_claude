...
class AnalyzerBot:
    ...
    def __init__(self, n_검증일수):
        ...
        self.folder_백테스팅 = os.path.join(dic_폴더정보['분석|백테스팅'], '돌파매매_종가매매')
    ...
    def pick_종목선정(self):
        ...
        li_전체일자 = sorted(re.findall(r'\d{8}', 파일)[0] for 파일 in os.listdir(folder_일봉) if '.pkl' in 파일)
        ...
        # 대상종목 선정 - 전일 조회순위 읽어와서 전일 조건으로 필터링 => 당일 실적 확인
        for s_일자 in li_대상일자:
            # 전일 확인
            ...
            # 소스 데이터 읽어오기 - 전일 조회순위
            df_조회순위 = pd.read_csv(os.path.join(folder_소스, f'{file_소스}_{s_전일}.csv'),
            ...
            df_조회순위 = df_조회순위.drop_duplicates(subset='종목코드').sort_values('시간').set_index('종목코드', drop=False)
            ...
            path_대상종목 = os.path.join(self.folder_서버, '데이터', '대상종목', f'df_대상종목_{s_전일}.pkl')
            ...
            for s_종목코드 in li_대상종목:
                ...
                b_전일거래증가 = n_전일거래량 > 1.5 * n_전일거래량ma20
                ...
                dic_종목선정.update(전일시가=n_전일시가, 전일고가=n_전일고가, 전일저가=n_전일저가, 전일종가=n_전일종가,
                                ...
                                전일변동성=n_전일변동성,
                                전일정배열=b_전일정배열, 전일양봉마감=b_전일양봉마감, 전일거래증가=b_전일거래증가,
            ...
            n_상승종목수 = len(df_종목선정[df_종목선정['당일수익']])
