...
class AnalyzerBot:
    ...
    def pick_종목선정(self):
        ...
        for s_일자 in li_대상일자:
            ...
            li_대상종목 = [종목 for 종목 in dic_일봉.keys() if 종목 in df_거래대상['종목코드'].values]
            li_조회순위 = [종목 for 종목 in li_대상종목 if 종목 in df_조회순위['종목코드'].values]
            ...
            for s_종목코드 in li_대상종목:
                ...
                n_전일거래대금 = df_일봉.loc[dt_전일, '거래대금(백만)']
                n_전일바디고가 = max(n_전일종가, n_전일시가)
                ...
                # b_종목선정 = b_전일돌파
                # b_종목선정 = b_일봉돌파 if b_조회순위포함 else (b_일봉돌파 and n_전일거래대금 > 1000*100)
                ...
                # b_종목선정 = b_전일돌파 and b_전일정배열 and b_전일거래증가
                ...
                n_전일고가대비상승률 = (n_당일고가 - n_전일고가) / n_전일고가 * 100
                n_고가20 = max(n_전일고가20, n_전일고가)
                n_고가20대비상승률 = (n_당일고가 - n_고가20) / n_고가20 * 100
                # b_당일수익 = ((n_당일고가상승률 - n_전일일봉고가율) > 5) and b_종목선정
                # b_당일수익 = n_전일고가대비상승률 > 3
                b_당일수익 = n_고가20대비상승률 > 3

                # 결과 생성
                dic_종목선정 = df_일봉.iloc[-1].to_dict()
                dic_종목선정.update(전일시가=n_전일시가, 전일고가=n_전일고가, 전일저가=n_전일저가, 전일종가=n_전일종가, 전일바디고가=n_전일바디고가,
                                전일고가20=n_전일고가20, 전일상승률=n_전일상승률,
                                전일ma5=n_전일ma5, 전일ma20=n_전일ma20, 전일ma120=n_전일ma120,
                                전일거래량=n_전일거래량, 전일거래량ma20=n_전일거래량ma20,
                                전일변동성=n_전일변동성,
                                전일돌파=b_전일돌파, 일봉돌파=b_일봉돌파, 전일정배열=b_전일정배열, 전일거래증가=b_전일거래증가,
                                전일양봉마감=b_전일양봉마감, 전일고가접근=b_전일고가접근, 전일변동제한=b_전일변동제한, 조회순위포함=b_조회순위포함,
                                전일거래대금=n_전일거래대금, 전일일봉고가=n_전일일봉고가, 전일일봉고가율=n_전일일봉고가율,
                                당일고가상승률=n_당일고가상승률, 전일고가대비상승률=n_전일고가대비상승률, 고가20대비상승률=n_고가20대비상승률,
                                당일수익=b_당일수익)
            ...
            #            .sort_values(['종목선정', '조회순위포함', '거래대금(백만)'], ascending=[False, False, False])
            #            .reset_index(drop=True)
            #            if len(li_dic종목선정) > 0 else pd.DataFrame())
            # df_종목선정 = (pd.DataFrame(li_dic종목선정)
            #            .sort_values(['전일돌파', '일봉돌파', '전일정배열', '전일거래증가', '조회순위포함', '전일상승률'],
            #                         ascending=[False, False, False, False, False, False])
            #            .reset_index(drop=True)
            #            if len(li_dic종목선정) > 0 else pd.DataFrame())
            df_종목선정 = pd.DataFrame(li_dic종목선정)
            # df_종목선정['돌파필터'] = df_종목선정['일봉돌파'] & df_종목선정['전일정배열']
            # df_종목선정['거래필터'] = df_종목선정['전일거래증가'] & df_종목선정['전일정배열']
            # df_종목선정['조회필터'] = df_종목선정['조회순위포함'] & df_종목선정['전일정배열']
            # df_종목선정 = (df_종목선정.sort_values(['돌파필터', '거래필터', '조회필터'], ascending=False)
            #            .reset_index(drop=True))
            df_종목선정['배열필터'] = df_종목선정['전일정배열']
            df_종목선정['조회필터'] = df_종목선정['조회순위포함']
            df_종목선정 = (df_종목선정.sort_values(['배열필터', '조회필터', '거래대금(백만)'], ascending=False)
                       .reset_index(drop=True))
            df_종목선정['종목선정'] = df_종목선정.index < 100
            ...
            # 로그 기록
            df_종목선정_조회순위 = df_종목선정.loc[df_종목선정['종목선정'] & df_종목선정['조회순위포함']]
            df_종목선정_추가선정 = df_종목선정.loc[df_종목선정['종목선정'] & ~df_종목선정['조회순위포함']]
            n_조회순위종목 = len(li_조회순위)
            n_선정종목_조회순위 = len(df_종목선정_조회순위)
            n_선정종목_추가선정 = len(df_종목선정_추가선정)
            n_수익종목_조회순위 = len(df_종목선정_조회순위[df_종목선정_조회순위['당일수익']])
            n_수익종목_추가선정 = len(df_종목선정_추가선정[df_종목선정_추가선정['당일수익']])
            n_성공률_조회순위 = n_수익종목_조회순위 / n_선정종목_조회순위 * 100 if n_선정종목_조회순위 != 0 else 0
            n_성공률_전체 = ((n_수익종목_조회순위 + n_수익종목_추가선정) / (n_선정종목_조회순위 + n_선정종목_추가선정) * 100
                        if (n_선정종목_조회순위 + n_선정종목_추가선정) != 0 else 0)
            self.make_로그(f'{s_일자}\n'
                         f' - {n_수익종목_조회순위:,.0f}+{n_수익종목_추가선정:,.0f}'
                         f' / {n_선정종목_조회순위:,.0f}+{n_선정종목_추가선정:,.0f}'
                         f' / {n_조회순위종목:,.0f}종목 - {n_성공률_조회순위:,.0f}%->{n_성공률_전체:,.0f}%')
    ...
    def make_매매정보(self):
        ...
        for s_일자 in li_대상일자:
            ...
            li_대상종목 = df_종목선정.loc[df_종목선정['종목선정'] & df_종목선정['조회순위포함']]['종목코드'].tolist()
    ...
    def _make_매매정보_종목(self, dic_매개변수):
        ...
        n_전일바디고가 = df_종목선정.loc[s_종목코드, '전일바디고가']
        n_전일종가 = df_종목선정.loc[s_종목코드, '전일종가']
